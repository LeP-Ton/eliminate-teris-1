...
## 当前变更文档
`workflow/20260616094755-restore-other-model-custom-popup.md` - 按用户要求放弃当前这轮下拉样式/交互实验，直接回退到另一模型实现的原始自定义弹窗版本。
`workflow/20260616094115-custom-dropdown-rect-panel-style.md` - 按用户要求把自定义下拉收敛为“只修样式”的简单版本：保留自定义选择框，浮层改为颜色协调的矩形面板，不再使用复杂切角结构。
`workflow/20260616093503-custom-dropdown-popover-timing-regression-fix.md` - 修复赛博朋克样式回归后自定义下拉再次不弹出的问题：恢复 `mouseUp + 下一轮 run loop` 的稳定 popover 展开时序。
`workflow/20260616092752-custom-dropdown-cyberpunk-restyle.md` - 保留当前可用的自定义下拉交互结构，只重做选择框与浮层视觉：恢复霓虹赛博朋克按钮、科技切角浮层和 hover/选中态。
`workflow/20260615232534-window-ui-cyberpunk-restyle.md` - 参考 `cyberpunk-ui` 将游戏窗口 UI 改造为赛博朋克风格：统一霓虹配色、科技切角卡片、扫描线背景、发光控件与更强的标题层级。
...
## 读取场景
- 需要确认“为什么最终不继续修当前实验版，而是直接回到另一模型实现的自定义弹窗版本”时，优先读取 `20260616094755` 文档。
- 需要确认“为什么最后放弃复杂赛博朋克切角浮层，改成简单矩形浮层，以及这版具体收敛了什么”时，优先读取 `20260616094115` 文档。
- 需要确认“为什么下拉样式改回赛博朋克后浮层又不弹了，以及如何修复这次时序回归”时，优先读取 `20260616093503` 文档。
- 需要确认“另一个模型已让自定义下拉能用，但样式不好看，后来如何只重做视觉、不推翻交互”时，优先读取 `20260616092752` 文档。
- 需要确认“窗口 UI 是否已经参考 `cyberpunk-ui` 做成赛博朋克风格、具体改了哪些桌面视觉层”时，优先读取 `20260615232534` 文档。
...
## 关键记忆
- 当前 `Sources/ArcadeCustomPopup.swift` 已按最新要求直接回退到另一模型实现的原始自定义弹窗版本；这轮所有样式实验都不再作为当前正式实现。
- 当前自定义下拉最新已按用户要求收敛为简单稳定版：保留自定义选择框，浮层使用与选择框配色一致的矩形/圆角矩形面板，不再使用复杂切角背景。
- `Sources/ArcadeCustomPopup.swift` 这套当前生效的自定义下拉，也会踩到 `transient NSPopover + mouseDown` 的老问题；现已同样改为 `mouseUp` 后异步切换展开状态。
- 当前真正生效的自定义下拉实现文件是 `Sources/ArcadeCustomPopup.swift`；最新已在不推翻现有弹出逻辑的前提下，把选择框与浮层样式重新拉回赛博朋克风格。
- 游戏窗口 UI 现已按赛博朋克方向重绘：桌面主舞台使用深色渐变 + 霓虹辉光 + 扫描线，卡片改为科技切角描边，控件改为霓虹描边发光风格，标题与徽标层级也同步增强。
